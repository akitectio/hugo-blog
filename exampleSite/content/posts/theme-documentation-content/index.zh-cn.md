...
了解如何在 **hugo-blog** 主题中快速, 直观地创建和组织内容.
...
- **linkToReport**: {{< version 0.2.14 >}} 如果设为 `false`, 则关闭页脚 **报告问题** 的链接. 你可以将其设置为一个用于报告此页面中错误的链接. 使用魔法变量 `{path}` 来获取文章的相对路径, 这篇文章的 `{path}` 是 `posts/theme-documentation-content/index.en.md`, 使用 `{title}` 来获取文章的标题, 这篇文章的 `{title}` 为 `Theme Documentation - Content`, 使用 `{url}` 来获取文章的链接, 这篇文章的 `{url}` 为 `https://hugo-blog.akitect.io/theme-documentation-content/`.
...
**hugo-blog** 主题使用内容摘要在主页中显示大致文章信息. Hugo 支持生成文章的摘要.
...
你仍然需要在文章开头添加 `<!--more-->` 摘要分割符. 将摘要分隔符之前的内容保留为空. 然后 **hugo-blog** 主题会将你的文章描述作为摘要.
...
**hugo-blog** 基于 [$ \KaTeX $](https://katex.org/) 提供数学公式的支持.
...
{{< admonition info >}}
你可以使用 [`math` shortcode](../theme-documentation-extended-shortcodes/#14-math) 以避免特殊字符造成[问题](https://github.com/akitectio/hugo-blog/issues/126).
{{< /admonition >}}
...
**hugo-blog** 主题支持一种 **字符注音或者注释** Markdown 扩展语法:
...
**hugo-blog** 主题支持一种 **分数** Markdown 扩展语法:
...
**hugo-blog** 主题使用 [Font Awesome](https://fontawesome.com/) 作为图标库.
你同样可以在文章中轻松使用这些图标.
